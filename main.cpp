...
int main()
{
    InitWindow(WIDTH, HEIGHT, "Basket Bounce");
    InitAudioDevice();
    SetTargetFPS(FPS);
    ...
    while (!WindowShouldClose())
    {
        ...
        if (!startGame)
        {
            DrawTextEx(f, "Basket Bounce", Vector2(WIDTH / 2. - 115., HEIGHT / 4. + 50.), 40., 0.0, WHITE);
        ...
        {
            float t = i / force;
            float x = balls.front()->pos.x + -force * std::cos(velo_angle) * i;
            float y = balls.front()->pos.y - (-force * std::sin(velo_angle) * i - 0.5 * grav * t * t);
            if (hold)
            {
                DrawCircle(x + 15, y + 15, 2., YELLOW);
            }
        }
        // akurat tapi jitter
        // for (int i = 0; i < 100; i++)
